...
public final class SREKFWrapper implements AutoCloseable, com.example.filtertest.KalmanFilterModule {

    /* ── customise these two for each variant ─────────────────────────── */
    private static final String LIB_NAME = "kalman_srekf";   // DLL / SO base name
    private static final String PREFIX   = "srekf";          // "srekf", "ukf", "srukf"
    ...
    @Override public void update(double x, double y) {
        try {
            if (!initialised) {
                ...
                System.out.printf("SREKFpreupdate: %.2f, %.2f%n", x, y);

                initialised = true;
            }
            UPDATE.invokeExact(handle, x, y);
            System.out.printf("SREKFpostupdate: %.2f, %.2f%n", x, y);
