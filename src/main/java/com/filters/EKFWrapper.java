...
public final class EKFWrapper implements AutoCloseable, com.example.filtertest.KalmanFilterModule {

    /* ── customise these two for each variant ─────────────────────────── */
    private static final String LIB_NAME = "kalman_ekf";   // DLL / SO base name
    private static final String PREFIX   = "ekf";          // "srekf", "ukf", "srukf"
...
/* first update sets the initial state */ }
    ...
    @Override public void update(double x, double y) {
        try {
            if (!initialised) {
                ...
                System.out.printf("pre update EKF: %.2f, %.2f%n", x, y);

                initialised = true;
            }
            UPDATE.invokeExact(handle, x, y);
            System.out.printf("post update EKF: %.2f, %.2f%n", x, y);
            pull();
        } catch (Throwable t) {
